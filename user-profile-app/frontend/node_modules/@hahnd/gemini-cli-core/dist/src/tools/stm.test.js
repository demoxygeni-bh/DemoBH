...
import { AddStmTool, SearchStmTool, DeleteStmTool, ClearStmTool, } from './stm.js';
...
describe('STM Tools', () => {
    ...
    describe('AddStmTool', () => {
        it('should add a new entry to an empty STM file', async () => {
            ...
            expect(writeFileSync).toHaveBeenCalledWith(mockStmFilePath, JSON.stringify(mockStmContent, null, 2), 'utf-8');
            expect(mockStmContent[0]).toEqual(expect.objectContaining({
                content,
            ...
            expect(result.llmContent).toBe(process.env.STM_SHOW_STATUS === 'TRUE'
        ...
        it('should add a new entry to an existing STM file', async () => {
            ...
            const result = await tool.execute({ content }, new AbortController().signal);
            expect(writeFileSync).toHaveBeenCalledWith(mockStmFilePath, JSON.stringify(mockStmContent, null, 2), 'utf-8');
            expect(mockStmContent).toEqual(expect.arrayContaining([
            ...
            expect(mockStmContent.find((e) => e.content === content)?.id).toMatch(/^00000000-0000-4000-8000-/);
            expect(result.llmContent).toBe(process.env.STM_SHOW_STATUS === 'TRUE'
    ...
    describe('SearchStmTool', () => {
            ...
            mockStmContent = [
                {
                    ...
                    created_at: formatDateToYYYYMMDD(new Date('2024-01-01T00:00:00.000Z')),
                    viewed_at: formatDateToYYYYMMDD(new Date('2024-01-01T00:00:00.000Z')),
                },
                {
                    id: 'id-2',
                    content: 'orange apple',
                    created_at: formatDateToYYYYMMDD(new Date('2024-01-02T00:00:00.000Z')),
                    viewed_at: formatDateToYYYYMMDD(new Date('2024-01-02T00:00:00.000Z')),
                },
                {
                    id: 'id-3',
                    content: 'grape',
                    created_at: formatDateToYYYYMMDD(new Date('2024-01-03T00:00:00.000Z')),
                    viewed_at: formatDateToYYYYMMDD(new Date('2024-01-03T00:00:00.000Z')),
                },
                {
                    id: 'id-4',
                    content: 'apple pie',
                    created_at: formatDateToYYYYMMDD(new Date('2024-01-04T00:00:00.000Z')),
        ...
        it('should return no entries if STM file does not exist', async () => {
            vi.mocked(existsSync).mockReturnValue(false);
            const tool = new SearchStmTool();
            const result = await tool.execute({ query: 'test' }, new AbortController().signal);
            expect(result.llmContent).toBe('Memory file does not exist. No entries to search.');
        });
        it('should search by id and update viewed_at', async () => {
            const tool = new SearchStmTool();
            vi.advanceTimersByTime(1000); // Advance time to ensure viewed_at is updated
            const result = await tool.execute({ id: 'id-2' }, new AbortController().signal);
            const parsedResult = process.env.STM_SHOW_STATUS === 'TRUE'
                ? JSON.parse(result.llmContent)
                : [];
            ...
            expect(typeof parsedResult[0].viewed_at).toBe('string');
            expect(mockStmContent[1].viewed_at).toBe(formatDateToYYYYMMDD(new Date()));
        });
        it('should search by query and return top 3 relevant entries and update viewed_at', async () => {
            const tool = new SearchStmTool();
            vi.advanceTimersByTime(1000); // Advance time to ensure viewed_at is updated
            const result = await tool.execute({ query: 'apple' }, new AbortController().signal);
            const parsedResult = process.env.STM_SHOW_STATUS === 'TRUE'
                ? JSON.parse(result.llmContent)
                : [];
            expect(parsedResult).toHaveLength(3);
            expect(parsedResult.map((entry) => entry.id)).toEqual([
                'id-1',
                'id-2',
                'id-4',
            ]);
            // Check if viewed_at is updated for returned results
            expect(mockStmContent[0].viewed_at).toBe(formatDateToYYYYMMDD(new Date()));
            expect(mockStmContent[1].viewed_at).toBe(formatDateToYYYYMMDD(new Date()));
            expect(mockStmContent[3].viewed_at).toBe(formatDateToYYYYMMDD(new Date()));
        });
        it('should return no matching entries found', async () => {
            const tool = new SearchStmTool();
            const result = await tool.execute({ query: 'xyz' }, new AbortController().signal);
            expect(result.llmContent).toBe('No matching memories entries found. The search returns occurrences containing all query words, case-insensitive. Consider trying broader terms or alternative queries to locate the memory.');
        });
        ...
        it('should search by keywords and return relevant entries', async () => {
            ...
            const tool = new SearchStmTool();
            const result = await tool.execute({ query: 'color blue' }, new AbortController().signal);
            const parsedResult = JSON.parse(result.llmContent);
            expect(parsedResult).toHaveLength(1);
            expect(parsedResult.map((entry) => entry.id)).toEqual(['id-5']);
        });
    });
    describe('DeleteStmTool', () => {
        ...
        it('should delete an existing entry by ID', async () => {
            ...
            const result = await tool.execute({ id: 'id-1' }, new AbortController().signal);
            expect(writeFileSync).toHaveBeenCalledWith(mockStmFilePath, JSON.stringify([
                {
                    id: 'id-2',
                    content: 'content 2',
                    created_at: '2024-01-02T00:00:00.000Z',
        ...
        it('should return message if STM file does not exist', async () => {
            ...
            const tool = new DeleteStmTool();
            const result = await tool.execute({ id: 'id-1' }, new AbortController().signal);
            expect(result.llmContent).toBe('Memory file does not exist. No entries to delete.');
        });
        it('should return message if entry not found', async () => {
            const tool = new DeleteStmTool();
            const result = await tool.execute({ id: 'non-existent-id' }, new AbortController().signal);
            expect(result.llmContent).toBe(process.env.STM_SHOW_STATUS === 'TRUE'
    ...
    describe('ClearStmTool', () => {
        ...
        it('should do nothing if STM file does not exist', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith('STM file does not exist. No entries to clear.');
        });
        it('should do nothing if no entries are older than 35 days', async () => {
            ...
            expect(writeFileSync).not.toHaveBeenCalled();
            expect(mockStmContent).toHaveLength(2);
            expect(console.log).toHaveBeenCalledWith('No STM entries older than 35 days found to clear.');
        });
    });
});
//# sourceMappingURL=stm.test.js.map