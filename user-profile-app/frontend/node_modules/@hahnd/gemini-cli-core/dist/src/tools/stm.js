...
export class AddStmTool extends BaseTool {
    constructor() {
        super('add_stm', 'Add memories', 'Saves memories to maintain context and enhance future interactions. Proactively record user preferences, project details, or key instructions. For optimal retrieval, use a `Key: Value` format (e.g., `User Name: John Doe`).', Icon.LightBulb, {
            type: Type.OBJECT,
            properties: {
                content: {
                    type: Type.STRING,
                    description: 'The memory content. Prefer `Key: Value` format (e.g., `User Name: Hermann Hahn`, `Project Context: Gemini CLI fork`)',
                },
            },
            required: ['content'],
...
export class SearchStmTool extends BaseTool {
    constructor() {
        super('search_stm', 'Search for memories', 'Searches memories for context. Use when understanding is uncertain. If initial query fails, try broader terms or multiple single-term queries. `query` parameter performs literal string match; it does not support boolean operators (e.g., `OR`).', Icon.FileSearch, {
            type: Type.OBJECT,
            properties: {
                query: {
                    type: Type.STRING,
                    description: 'Keywords or phrases to search memory content. Performs literal match; no boolean operators.',
                },
                id: {
                    type: Type.STRING,
    ...
    async execute(args, _signal) {
        ...
        const stmEntries = JSON.parse(readFileSync(stmFilePath, 'utf-8'));
        let results = [];
        if (args.id) {
            ...
            const scoredEntries = stmEntries.map((entry) => {
                ...
                const allWordsPresent = lowerCaseQueryWords.every((word) => lowerCaseContent.includes(word));
                if (allWordsPresent) {
 ...
 */
export class DeleteStmTool extends BaseTool {
    constructor() {
        super('delete_stm', 'Delete memories', 'Deletes a memory entry by its ID. Use for irrelevant/outdated entries or user-requested deletion.', Icon.Trash, {
            type: Type.OBJECT,
            properties: {
                id: {
                    type: Type.STRING,
    ...
    async execute(args, _signal) {
        ...
        let stmEntries = JSON.parse(readFileSync(stmFilePath, 'utf-8'));
...
export class ClearStmTool {
    async execute() {
        ...
        const initialLength = stmEntries.length;
        stmEntries = stmEntries.filter((entry) => entry.viewed_at > thirtyFiveDaysAgoFormatted);
        if (stmEntries.length === initialLength) {
            console.log('No STM entries older than 35 days found to clear.');
            return;
        }
        writeFileSync(stmFilePath, JSON.stringify(stmEntries, null, 2), 'utf-8');
        console.log(`Cleared ${initialLength - stmEntries.length} old STM entries.`);
    }
}
